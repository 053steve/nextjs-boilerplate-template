...
const Alert: React.FC = () => {
    // const {message, showMessage} = useSelector((state): AppState => state);
    //
    // console.log(message);
    // console.log(showMessage);
    const common = useSelector((state): CommonState => state.common);

    const {message, loading, showMessage} = common;

    let alertTab;
    if (showMessage) {
        alertTab = <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
